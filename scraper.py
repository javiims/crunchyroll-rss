...
def process_url(page, url):
    """Detecta el audio leyendo el texto plano de la página renderizada."""
    page.goto(url, wait_until="networkidle", timeout=120000)
    
    # Extraemos el texto visible de toda la página
    text = page.locator("body").inner_text()
    
    # Limpiamos espacios extra para asegurar la detección
    text = " ".join(text.split())

    # 1. Detectar Audiodescripción (Prioridad alta)
    if "Español (España) [descripción de audio]" in text:
        return "Audiodescripción"
    
    # 2. Detectar Audio Normal
    # Buscamos 'Español (España)' pero asegurando que no estamos mirando la sección de Subtítulos
    # Dividimos el texto para aislar la sección de 'Idiomas de audio'
    if "Idiomas de audio" in text:
        audio_section = text.split("Idiomas de audio")[1].split("Subtítulos")[0]
...
def main():
    ...
    with Stealth().use_sync(sync_playwright()) as p:
        ...
        for url in urls:
            ...
            try:
                title = extract_title(page) # Obtenemos título primero
